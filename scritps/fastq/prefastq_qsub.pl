...
for (my $i=0;$i<@fq;$i++){
   ...
   if ($fq[$i]=~/(.*\/RIL(\d+)\_\d+\_*\w*)_p1\.fq/){
      ...
      #`ln -s $fq[$i] $file1`;
      #`ln -s $fq2 $file2`;
      my $cmd1="/rhome/cjinfeng/software/tools/seqtk-master/seqtk sample -s100 $fq[$i] 350000 > $file1";
      my $cmd2="/rhome/cjinfeng/software/tools/seqtk-master/seqtk sample -s100 $fq2 350000 > $file2";
      print OUT "$cmd1\n$cmd2\n";
...
`perl /rhome/cjinfeng/software/bin/qsub-pbs.pl subfastq.sh > log 2> log2`;
